...
{
    public static class Armor3Script
    {
        public static void Armor3(ScriptUtility su)
        {
            Data data = DataProvider.Get();

            //Returns when subscript "Armor3" is disabled
            if (!data.Settings.IsArmor3On)
                return;

            //Backs DPI on end of script
            ScriptsManager.GetScriptByName("Armor3")!.EndWithMethod += DPIUtility.BackDPI;
            ...
            Point helmetEQ = new(X, (int)(Y + slotY * 7.5));
            Point chestplateEQ = new(X + slotX, (int)(Y + slotY * 7.5));
            Point leggingsEQ = new(X + slotX * 2, (int)(Y + slotY * 7.5));
            Point bootsEQ = new(X + slotX * 3, (int)(Y + slotY * 7.5));
