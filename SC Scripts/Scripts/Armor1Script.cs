...
{
    public static class Armor1Script
    {
        public static void Armor1(ScriptUtility su)
        {
            Data data = DataProvider.Get();

            //Returns when subscript "Armor1" is disabled
            if (!data.Settings.IsArmor1On)
                return;

            //Backs DPI on end of script
            ScriptsManager.GetScriptByName("Armor1")!.EndWithMethod += DPIUtility.BackDPI;
            ...
            Point helmetEQ = new(X, (int)(Y + slotY * 5.5));
            Point chestplateEQ = new(X + slotX, (int)(Y + slotY * 5.5));
            Point leggingsEQ = new(X + slotX * 2, (int)(Y + slotY * 5.5));
            Point bootsEQ = new(X + slotX * 3, (int)(Y + slotY * 5.5));
